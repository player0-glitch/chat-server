...
int main(int argc, char *argv[]) {
    ...
    std::thread send_thread(send_msg, socket_fd);
    std::thread receive_thread(receive_msg, socket_fd);
    ...
    if (!is_socket_connected(socket_fd)) {
        ...
        cout << "gracefully close the socket"; // Im probably lying out my ass here
    }
    return 0;
}
 ...
 */
void help(int argc) {
    cerr << "Incorrect amount of command line arguments " << argc << "expected 3\n";
